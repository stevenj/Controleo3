...
#define DEQUANTIZE(coef, quantval) DESCALE((coef) * (quantval), IFAST_SCALE_BITS - PASS1_BITS)
       ...
       GLOBAL(void) jpeg_idct_ifast(j_decompress_ptr cinfo, jpeg_component_info *compptr, JCOEFPTR coef_block,
                                    JSAMPARRAY output_buf, JDIMENSION output_col)
{
	...
	for (ctr = 0; ctr < DCTSIZE; ctr++) {
		...
		z5 = (DCTELEM)wsptr[0] + ((((DCTELEM)RANGE_CENTER) << (PASS1_BITS + 3)) + (1 << (PASS1_BITS + 2)));
