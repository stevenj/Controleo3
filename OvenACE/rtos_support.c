...
static TaskHandle_t      xCreatedExampleTask2;
...
static void example_task2(void *p)
{
//	(void)p;
	while (1) {
//		if (xSemaphoreTake(disp_mutex, ~0)) {
...
void FREERTOS_V1000_0_example(void)
{
	...
	if (xTaskCreate(
	        example_task2, "Example2", TASK_EXAMPLE_STACK_SIZE, NULL, TASK_EXAMPLE_STACK_PRIORITY, xCreatedExampleTask2)
	    != pdPASS) {
		while (1) {
			;
		}
